...
pub const SparseModule = struct {
    ...
    const Code = struct {
        locals: std.ArrayListUnmanaged(types.ValType),
        expr: Expr,
    };
    ...
    memories: std.ArrayListUnmanaged(types.Limits) = .empty,
    tags: std.ArrayListUnmanaged(types.TagType) = .empty,
    ...
    fn decodeMemorySec(self: *SparseModule, reader: *std.Io.Reader) !void {
        ...
        for (0..memory_count) |_| {
            self.memories.append(gpa, try types.Limits.decode(&data_reader)) catch unreachable;
        }
    ...
    fn decodeTagSec(self: *SparseModule, reader: *std.Io.Reader) !void {
        ...
        for (0..tag_count) |_| {
            self.tags.append(gpa, try types.TagType.decode(&data_reader)) catch unreachable;
        }
    ...
    fn decodeGlobalSec(self: *SparseModule, reader: *std.Io.Reader) !void {
        ...
        for (0..global_count) |_| {
            const global_type = try types.GlobalType.decode(&data_reader);
            const expr = try Expr.decode(gpa, &data_reader);
    ...
    fn decodeExportSec(self: *SparseModule, reader: *std.Io.Reader) !void {
    ...
    fn decodeStartSec(self: *SparseModule, reader: *std.Io.Reader) !void {
    ...
    fn decodeElemSec(self: *SparseModule, reader: *std.Io.Reader) !void {
        ...
        for (0..elem_count) |_| {
            ...
            const elem: Elem = switch (flags) {
                ...
                5 => .{
                    .type_info = .{ .ref_type = try types.RefType.decode(&data_reader) },
                    .init = .{ .exprs = try decodeElemExprList(gpa, &data_reader) },
                    .mode = .passive,
                },
                6 => blk: {
    ...
    fn decodeDataCountSec(self: *SparseModule, reader: *std.Io.Reader) !void {
    ...
    fn decodeCodeSec(self: *SparseModule, reader: *std.Io.Reader) !void {
        ...
        for (0..code_count) |_| {
            ...
            for (0..local_decl_count) |_| {
                ...
                const val_type = try types.ValType.decode(&code_reader);
                for (0..n) |_| {
    ...
    fn decodeDataSec(self: *SparseModule, reader: *std.Io.Reader) !void {
